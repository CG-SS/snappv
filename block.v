// Snappy compression algorithm idiomatic V implementation, therefore called "Snappv".
//
...
pub fn decompress(input []u8) ![]u8 {
	...
	if !ok {
		return error('snappv: invalid header varint')
	}

	// Pre-allocate the exact output size; reject obviously bogus lengths.
	ilen := int(ulen)
	if ilen < 0 {
		return error('snappv: decoded length overflow')
	}
	...
	for pos < input.len {
		...
		if tag_type == 0 {
			...
			if !lit_ok {
				return error('snappv: truncated literal length')
			}
			pos = new_pos
			if pos + lit_len > input.len {
				return error('snappv: literal extends past input')
			}
			if op + lit_len > ilen {
				return error('snappv: output exceeds declared length')
			}
			for i in 0 .. lit_len {
			...
			if pos >= input.len {
				return error('snappv: truncated copy-1 tag')
			}
			length := int((tag >> 2) & 0x07) + 4
			offset := int(u32(tag & 0xe0) << 3 | u32(input[pos]))
			pos++
			if offset == 0 {
				return error('snappv: zero offset in copy-1')
			}
			op = expand_copy_into(mut out, op, ilen, offset, length)!
		} else if tag_type == 2 {
			// COPY_2 (3-byte tag)
			if pos + 1 >= input.len {
				return error('snappv: truncated copy-2 tag')
			}
			length := int((tag >> 2) & 0x3f) + 1
			offset := int(u32(input[pos]) | u32(input[pos + 1]) << 8)
			pos += 2
			if offset == 0 {
				return error('snappv: zero offset in copy-2')
			}
			op = expand_copy_into(mut out, op, ilen, offset, length)!
		} else {
			// COPY_4 (5-byte tag)
			if pos + 3 >= input.len {
				return error('snappv: truncated copy-4 tag')
			}
			length := int((tag >> 2) & 0x3f) + 1
			...
			pos += 4
			if offset == 0 {
				return error('snappv: zero offset in copy-4')
			}
			op = expand_copy_into(mut out, op, ilen, offset, length)!
		}
	}
	...
	if op != ilen {
		return error('snappv: decompressed length mismatch: got ${op}, want ${ilen}')
	}
	return out
}
...
fn expand_copy_into(mut out []u8, op int, cap_ int, offset int, length int) !int {
	src := op - offset
	if src < 0 {
		return error('snappv: copy offset past beginning of output')
	}
	if op + length > cap_ {
		return error('snappv: output exceeds declared length')
	}
