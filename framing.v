...
//
// Spec: https://github.com/google/snappv/blob/main/framing_format.txt
//
...
pub fn decode_stream(input []u8) ![]u8 {
	...
	for pos < input.len {
		...
		if pos + chunk_len > input.len {
			return error('snappv framing: chunk body truncated at pos ${pos}')
		}
		...
		if chunk_type == chunk_type_compressed {
			...
			return error('snappv framing: unsupported unskippable chunk type 0x${chunk_type:02x}')
		}
	}
...
pub fn (mut enc StreamEncoder) write(buf []u8) !int {
	if enc.closed {
		return error('snappv: write on a closed stream encoder')
	}
...
pub fn (mut dec StreamDecoder) write(buf []u8) !int {
	if dec.closed {
		return error('snappv: write to closed stream decoder')
	}
	if err := dec.err {
		return err
	}
	...
	for {
		...
		chunk_type, chunk_len, hdr_end := read_chunk_header(dec.buf, dec.offset) or {
			dec_err := error('snappv: failed while reading chunk header: ${err}')
			dec.err = dec_err
			return dec_err
		}
		...
		if !dec.identified {
			if chunk_type != chunk_type_stream_id {
				dec_err := error('snappv framing: stream does not begin with identifier chunk')
				dec.err = dec_err
				return dec_err
			}
			if chunk_len != stream_identifier_magic.len {
				dec_err := error('snappv framing: bad identifier chunk length ${chunk_len}')
				dec.err = dec_err
				return dec_err
			}
			if chunk_data != stream_identifier_magic {
				dec_err := error('snappv framing: bad stream identifier payload')
				dec.err = dec_err
				return dec_err
			}
			dec.identified = true
			...
			uncompressed := decode_compressed_chunk(chunk_data) or {
				dec_err := error('snappv: failed while decoding compressed chunk: ${err}')
				dec.err = dec_err
				return dec_err
			}
			...
			raw := decode_uncompressed_chunk(chunk_data) or {
				dec_err := error('snappv: failed while decoding uncompressed chunk: ${err}')
				dec.err = dec_err
				return dec_err
			}
			dec.output << raw
			...
			dec_err := error('snappv framing: unsupported unskippable chunk type 0x${chunk_type:02x}')
			dec.err = dec_err
			return dec_err
		}
...
fn read_stream_identifier(data []u8, pos int) !int {
	...
	if chunk_type != chunk_type_stream_id {
		return error('snappv framing: stream must begin with identifier chunk, got 0x${chunk_type:02x}')
	}
	if chunk_len != stream_identifier_magic.len {
		return error('snappv framing: bad identifier chunk length ${chunk_len}')
	}
	if new_pos + chunk_len > data.len {
		return error('snappv framing: identifier chunk truncated')
	}
	magic := data[new_pos..new_pos + chunk_len]
	if magic != stream_identifier_magic {
		return error('snappv framing: bad stream identifier magic')
	}
...
fn read_chunk_header(data []u8, pos int) !(u8, int, int) {
	if pos + 4 > data.len {
		return error('snappv framing: truncated chunk header at pos ${pos}')
	}
	chunk_type := data[pos]
...
fn decode_compressed_chunk(payload []u8) ![]u8 {
	if payload.len < 4 {
		return error('snappv framing: compressed chunk too short')
	}
	...
	if actual_crc != stored_masked_crc {
		return error('snappv framing: CRC32C mismatch in compressed chunk')
	}
	return uncompressed
}
...
fn decode_uncompressed_chunk(payload []u8) ![]u8 {
	if payload.len < 4 {
		return error('snappv framing: uncompressed chunk too short')
	}
	stored_masked_crc := u32(payload[0]) | u32(payload[1]) << 8 | u32(payload[2]) << 16 | u32(payload[3]) << 24
	raw := payload[4..].clone()
	actual_crc := mask_crc(crc32c(raw))
	if actual_crc != stored_masked_crc {
		return error('snappv framing: CRC32C mismatch in uncompressed chunk')
	}
	return raw
}
